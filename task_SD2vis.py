#############################################################################
#
#    SD2VIS: Generating synthetic visibilities from TP images.
#
...
#  SDimage            =  "/home/guest31/data/JK_data_2016.1.01162.S/G357_a_03_TP/2016.1.01162.S/science_goal.uid___A001_X87c_X12/group.uid___A001_X87c_X13/member.uid___A001_X87c_X1c/product/G357.spw17.I.sd.im.fits"
...
def SD2vis(SDimage='', SDchannels = -1, # NuReGrid=[], 
  ...
  units = {'deg':3600.,'rad':180./np.pi*3600.,'arcsec':1.,'arcmin':60.,'mas':0.001}
  sig2FWHM = 2.35482
  util = simutil('')
...
# Same as above, but raise an exception afterwards:
  def printErr(msg):
    print msg
...
# Draw numbers from a 2D Gaussian distribution, but
# within a range of allowed radii:
...
# SANITY CHECKS:
  ...
  if not os.path.exists(inputvis):
    printMsg('WARNING! No inputvis is given! Only one pointing will be simulated!')
...
# PREPARE IMAGE (REGRID AND RE-ARRANGE AXES):
  ...
  if len(NuReGrid)==0:
    ...
    try:
      printMsg('Will regrid image to Nu0: %.8e GHz, dNu: %.8e MHz and %i channels'%(NuReGrid[0]/1.e9,NuReGrid[1]/1.e6,NuReGrid[2]))
    except:
    ...
    ia.regrid(outfile='%s.COPY'%SDimage,csys=myrec,shape=Shape,overwrite=True)
    ia.close()
...
# CONVERT INTO A CASA IMAGE:
  ...
  try: # A FITS
    ...
    testf.close()
    importfits(fitsimage='%s.COPY'%SDimage,imagename='SD2VIS.TEMP',overwrite=True)
...
#  ia.close()
  ...
  NPIX = summ['shape'][:2]
  INCR = [ic*units[summ['axisunits'][0]]/units['rad'] for ic in summ['incr'][:2]]
...
#    printErr('THIS IS NOT AN IMAGE CUBE!')
  ...
  Freqs = (np.arange(summ['shape'][nudim]) + summ['refpix'][nudim])*summ['incr'][nudim]+summ['refval'][nudim]
...
# We assume a square image:
  imsize = np.abs(summ['incr'][0])*summ['shape'][0]*units[summ['axisunits'][0]]/units['rad']  # in radians
...
#    psize = np.abs(summ['incr'][0]*units[summ['axisunits'][0]]/units['rad'])
#    print 'PIXEL SIZE: ',psize
# By now, assume a circular TP beam:
...
# image pixel coordinates:
  xx = (np.arange(summ['shape'][0]) - summ['refpix'][0])*summ['incr'][0]*units[summ['axisunits'][0]]/units['rad']
  npix = summ['shape'][0]
...
#    datfft[:,:,i] = np.fft.fftshift(np.fft.fft2(data[:,:,i]))
#  else:
  for sti in range(nst):
#    if nudim==3:
    for i in range(SDchans[0],SDchans[1]):
      datfft[:,:,sti,i] = np.fft.fftshift(np.fft.fft2(data[:,:,sti,i]))
#    else:
#     for i in range(SDchans[0],SDchans[1]):
#      datfft[:,:,i,sti] = np.fft.fftshift(np.fft.fft2(data[:,:,i,sti]))
...
# UNCONVOLVE TP BEAM:
  if Fsigma>0.0:
    ...
    UU = np.linspace(-UVsize,UVsize,int(summ['shape'][0]),endpoint=False)
    Uones = np.ones(summ['shape'][1])

    VV = np.linspace(-UVsize,UVsize,int(summ['shape'][1]),endpoint=False)
...
#    if ndim==3:
#     for i in range(SDchans[0],SDchans[1]):
#      data[:,:,i] = np.fft.ifft2(np.fft.ifftshift(datfft[:,:,i]*UVTaper)).real
  
#    else:
    for sti in range(nst):
#      if nudim==3:
      for i in range(SDchans[0],SDchans[1]):
        data[:,:,sti,i] = np.fft.ifft2(np.fft.ifftshift(datfft[:,:,sti,i]*UVTaper)).real
 
#      else:
#       for i in range(SDchans[0],SDchans[1]):
#        data[:,:,i,sti] = np.fft.ifft2(np.fft.ifftshift(datfft[:,:,i,sti]*UVTaper)).real
...
#  antn = ['SD%02i'%i for i in range(len(stx))]
...
#  sm.setspwindow(spwname='TP0',freq='%.8fGHz'%(np.min(Freqs)/1.e9),
#       deltafreq='%.9fGHz'%(np.abs(summ['incr'][nudim])/1.e9),
#       freqresolution = '%.9fGHz'%(np.abs(summ['incr'][nudim])/1.e9),
#       nchannels=len(Freqs), refcode=outframe, stokes='XX YY')
  ...
  RA = summ['refval'][0] ; Dec = summ['refval'][1]
...
# Set pointings:
  if len(inputvis)>0:
    ...
    Pointings = ['%.16frad %.16frad'%(RAs[i],Decs[i]) for i in range(NPOINT)]
    tb.close()
  else:
...
#####################
  ...
  sm.settimes(integrationtime='%is'%tint,usehourangle=True, referencetime=mereftime)
...
# Select the whole spw if chans=[0,-1]
    ...
    if chans[1]>spchan:
      printErr('ERROR: There are only %i channels! Change the channel range!'%spchan)
    else:
      printMsg('Will use channels from %i to %i (included)'%(chans[0],chans[1]-1))

    Qs = intbas[0,:]**2. + intbas[1,:]**2.
    Q = np.logical_and(Qs>0.,Qs<scale**2.)
    Mask = np.logical_and(visdats[:,chans[0]:chans[1],Q]!=0.,tpdats[:,chans[0]:chans[1],Q]!=0.)
    if np.sum(Mask)>0:
      scale = np.average(np.abs(visdats[:,chans[0]:chans[1],Q][Mask])/np.abs(tpdats[:,chans[0]:chans[1],Q][Mask]))
    else:
      printMsg('WARNING: Did not find baselines as short as %.1f m.!'%(-scale))
      scale = 1.0
...
#    sm.close()
    ...
    Ulamb = 2.*np.pi*freqs[:,np.newaxis]/299792458.*tpdata['u'][np.newaxis,:]
    Vlamb = 2.*np.pi*freqs[:,np.newaxis]/299792458.*tpdata['v'][np.newaxis,:]
    tpdata['flag'] = False
    ...
    for k in range(nvis):
      ...
      for i in range(NPIX[0]):
        ...
        for j in range(NPIX[1]):
          ...
          TotVis[:] += data2sq[i,j,SDchans[0]:SDchans[1]]*np.exp(1.j*(RASum + Vlamb[:,k]*dDec))
